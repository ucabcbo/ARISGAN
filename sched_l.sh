...
if test -f "$experiment_path"; then
    ...
    echo "qsub -N "$job_name" -l h_rt="$duration_str",mem="$2"G,gpu=1,tmpfs=100G -ac allow=L -wd $working_dir -m es train.sh "$3" "$4
    qsub -N $job_name -l "h_rt="$duration_str",mem="$2"G,gpu=1,tmpfs=100G" -ac allow=L -wd $working_dir -m es train.sh $3 $4
else
