...
for file in selected_files:
    ...
    raw_tiff = rasterio.open(os.path.join(env.DATA_ROOT, f'tif{TILESIZE}/', file))

    subfolder = f'alt6/{TILESIZE}/{suffix}/'
    ...
    subfolder = f'alt12/{TILESIZE}/{suffix}/'
    if not os.path.exists(os.path.join(env.DATA_ROOT, subfolder)):
        os.makedirs(os.path.join(env.DATA_ROOT, subfolder))
    tensorpath = os.path.join(env.DATA_ROOT, subfolder, f'{filename}.tfrecord')
    if os.path.exists(tensorpath):
    ...
    subfolder = f'alt30/{TILESIZE}/{suffix}/'
    if not os.path.exists(os.path.join(env.DATA_ROOT, subfolder)):
        os.makedirs(os.path.join(env.DATA_ROOT, subfolder))
    tensorpath = os.path.join(env.DATA_ROOT, subfolder, f'{filename}.tfrecord')
    if os.path.exists(tensorpath):
    ...
    subfolder = f'curated/{TILESIZE}/{suffix}/'
    if not os.path.exists(os.path.join(env.DATA_ROOT, subfolder)):
        os.makedirs(os.path.join(env.DATA_ROOT, subfolder))
    tensorpath = os.path.join(env.DATA_ROOT, subfolder, f'{filename}.tfrecord')
    if os.path.exists(tensorpath):
